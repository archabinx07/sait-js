...
function sumOfArray(array) {
    ...
    for(var i = 0; i < array.length; i++) {
        if (array[i] <= 5) {
            Total += array[i];  // Iterate over your first array and then grab the second element add the values up
        }
    }
    return Total; 
}
...
shapes.hypo = Math.sqrt(Math.pow(triangle.base, 2) + Math.pow(triangle.height, 2));
console.log(shapes);